...
def getFileList(procData,DefaultNFilesPerJob):
   ...
   for sample in miniAODSamples:
      ...
      if(split>0):
         ...
         if(hostname.find("iihe.ac.be")!=-1): NFilesPerJob = DefaultNFilesPerJob
...
if(commands.getstatusoutput("hostname -f")[1].find("iihe.ac.be")!=-1): parser.add_option('-S', "--NFile"      ,    dest='NFile'              , help='default #Files per job (for autosplit)'    , default=4)
