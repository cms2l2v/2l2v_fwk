...
class BTagCalibrationReader80X::BTagCalibrationReader80XImpl
{
  friend class BTagCalibrationReader80X;
...
private:
  BTagCalibrationReader80XImpl(BTagEntry::OperatingPoint op,
  ...
  std::map<std::string, std::shared_ptr<BTagCalibrationReader80XImpl>> otherSysTypeReaders_;
};


BTagCalibrationReader80X::BTagCalibrationReader80XImpl::BTagCalibrationReader80XImpl(
                                             BTagEntry::OperatingPoint op,
                                             const std::string & sysType,
                                             const std::vector<std::string> & otherSysTypes):
  op_(op),
  ...
  useAbsEta_(3, true)
{
  for (const std::string & ost : otherSysTypes) {
    if (otherSysTypeReaders_.count(ost)) {
      throw cms::Exception("BTagCalibrationReader80X")
            << "Every otherSysType should only be given once. Duplicate: "
            << ost;
    }
    otherSysTypeReaders_[ost] = std::auto_ptr<BTagCalibrationReader80XImpl>(
        new BTagCalibrationReader80XImpl(op, ost)
    );
  }
}

void BTagCalibrationReader80X::BTagCalibrationReader80XImpl::load(
...
double BTagCalibrationReader80X::BTagCalibrationReader80XImpl::eval(
                                             BTagEntry::JetFlavor jf,
                                             float eta,
                                             float pt,
                                             float discr) const
...
double BTagCalibrationReader80X::BTagCalibrationReader80XImpl::eval_auto_bounds(
...
std::pair<float, float> BTagCalibrationReader80X::BTagCalibrationReader80XImpl::min_max_pt(
                                               BTagEntry::JetFlavor jf,
                                               float eta,
                                               float discr) const
{
...
BTagCalibrationReader80X::BTagCalibrationReader80X(BTagEntry::OperatingPoint op,
                                             const std::string & sysType,
                                             const std::vector<std::string> & otherSysTypes):
  pimpl(new BTagCalibrationReader80XImpl(op, sysType, otherSysTypes)) {}

void BTagCalibrationReader80X::load(const BTagCalibration & c,
                                 BTagEntry::JetFlavor jf,
...
double BTagCalibrationReader80X::eval(BTagEntry::JetFlavor jf,
                                   float eta,
                                   float pt,
                                   float discr) const
{
...
double BTagCalibrationReader80X::eval_auto_bounds(const std::string & sys,
                                               BTagEntry::JetFlavor jf,
                                               float eta,
                                               float pt,
                                               float discr) const
...
std::pair<float, float> BTagCalibrationReader80X::min_max_pt(BTagEntry::JetFlavor jf,
                                                          float eta,
                                                          float discr) const
{
