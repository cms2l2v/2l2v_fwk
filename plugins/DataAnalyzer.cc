...
DataAnalyzer::DataAnalyzer(const edm::ParameterSet &iConfig) : obsPU_h(0), truePU_h(0)
{
	...
	keepFullGenInfo = 0 ; keepFullGenInfo =  analysisCfg_.getParameter<bool>("keepFullGenInfo_");
	...
	string selSteps[]={"RECO","No scrapping","#geq 1 vertex","MET Filters"};
	int nselFilters = sizeof(selSteps)/sizeof(TString);
...
void DataAnalyzer::analyze(const edm::Event &event, const edm::EventSetup &iSetup) 
{
	...
	// GENERATOR LEVEL
	//
	int nHardProcGenLeptons(0);
	int nHardProcGenBosons(0);
	if(!isData){
		...
		for(size_t i = 0; i < genParticlesH->size(); ++ i)
		{
			...
			bool isStableOfInterest( keepFullGenInfo && p.status()==1 && ((abs(p.pdgId())==22 && p.pt()>20) || passLepAccCut  || (abs(p.pdgId()) >= 11 && abs(p.pdgId()) <= 14) ));
			if(!isHardProc && !isStableOfInterest) continue;
			// for Madgraph MC and pythia
			if( (abs(p.pdgId()) >= 11 && abs(p.pdgId()) <= 14)  && !passLepAccCut && p.numberOfMothers()  == 1 &&  p.mother()->pdgId() != p.pdgId() && p.mother()->pdgId() != 23 && abs (p.mother()->pdgId() ) != 24 ) continue;
                        // for Sherpa MC
                        if( (abs(p.pdgId()) >= 11 && abs(p.pdgId()) <= 14)  && !passLepAccCut && p.numberOfMothers()  == 2 && (abs(p.mother(0)->pdgId()) != abs(p.pdgId()) ||  abs(p.mother(1)->pdgId()) != abs(p.pdgId())) )  continue;

			nHardProcGenLeptons += (isHardProc && (abs(p.pdgId()) >= 11 && abs(p.pdgId()) <= 14 ));
	...
	// to keep full GEN info for DY and WJets decaying to muon/electron
	if(!isData && nHardProcGenLeptons > 0 && nHardProcGenBosons > 0 && keepFullGenInfo ) toSave=true;
	if(!toSave) return;
