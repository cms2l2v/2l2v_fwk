  ...
  for(unsigned int i=0;i<Process.size();i++)
    {
      ...
      cout << "Process named " << proc << " is data? " << isData << endl;
...
void getYieldsFromShapes(const map<TString, Shape_t> &allShapes)
{
  ...
  for(std::vector<int>::iterator bIt = binsToProject.begin(); bIt != binsToProject.end(); bIt++)
    {
      if(!dataTempl) cout << "Data template does not exist..." << endl;
  ...
  for(std::map<TString, Shape_t>::const_iterator it=allShapes.begin(); it!=allShapes.end(); it++)
    {
      ...
      fprintf(pFile,"%6s ",ch.Data());
      for(size_t j=0; j<shape.bckg.size(); j++) fprintf(pFile,"%6s ", ch.Data());
      fprintf(pFile,"\n");
...
//      fprintf(pFile,"\n");
	...
	{
	  continue; // Temporarily disable them
