...
int main(int argc, char* argv[]){
  ...
  std::vector<string> catL  = {"#mu#mu", "ee", "ll", "#gamma+jets"};
  std::vector<string> binL  = {"=0jet", "#geq1jets", "vbf"};

  std::vector<string> cat   = {"mumu", "ee", "ll", "photon"};
  ...
  for(unsigned int v=0;v<var.size();v++){
     string& DataFile             = (cat[c]=="photon")?gDataFile:zDataFile;
     std::vector<string>& DataDir = (cat[c]=="photon")?gDataDir :zDataDir;
...
//             TF1* fitFunc = new TF1("myPol2","pol2",60,500);
     ...
     c1->SaveAs((string("PhotonWeight")+var[v]+".png").c_str());
  }
