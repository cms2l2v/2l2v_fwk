...
LandSArgCommonOptions=" --blind --rebin 8 --dropBckgBelow 0.005 "
...
signalSuffixVec += [ "" ]
OUTName         += ["SB13TeV"]
LandSArgOptions += [" --bins eq0jets,geq1jets,vbf --systpostfix _13TeV --shape "]

#loop bellow is to run limit on narrow resonnance signal
#for CP in [-1.0, 1.0]:
#   for BR in [0.0]:  
...
#      OUTName         += ["SB13TeV"]
#      LandSArgOptions += [" --bins eq0jets --systpostfix _13TeV --shape "]
#
#      #ShapeBased GG only
#      signalSuffixVec += [suffix ]
#      OUTName         += ["SB13TeV_GG"]
#      LandSArgOptions += [" --bins eq0jets,geq1jets --skipQQH --systpostfix _13TeV --shape "]
#
#      #ShapeBased VBF only
#      signalSuffixVec += [suffix ]
#      OUTName         += ["SB13TeV_QQ"]
#      LandSArgOptions += [" --bins vbf              --skipGGH --systpostfix _13TeV --shape "]
...
cuts1   = file.Get('Z#rightarrow ll/optim_cut1_met') 
#cuts2   = file.Get('Z#rightarrow ll/optim_cut1_mtmin') 
#cuts3   = file.Get('Z#rightarrow ll/optim_cut1_mtmax') 
...
#         for mtmin in range(125,550,25):
         for mtmin in range(100,600,100):
            mtmin_ = mtmin
...
#            for mtmax in range(mtmin+50,mtmin+500,25):
            for mtmax in range(mtmin+50,mtmin+500,100):
               mtmax_ = mtmax
