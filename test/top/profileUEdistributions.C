...
void profileUE();
void drawCMSHeader(TString ch)
{
  TPaveText *pt=new TPaveText(0.06,0.90,0.9,0.99,"brNDC");
  pt->SetBorderSize(0);
  pt->SetFillStyle(0);
  pt->SetTextAlign(12);
  //  pt->SetTextSize(0.09);
...
void profileDistributions(TString dist)
{
  ...
  TFile *inF=TFile::Open("plotter.root");
  TFile *inSystF=TFile::Open("plotter_syst.root");
  ...
  //TString systList[]={"t#bar{t}systtunep11","t#bar{t}systtunep11nocr","t#bar{t}systtunep11tev"};
  //TString systLabels[]={"P11","P11-noCR","P11TeV"};
  ...
  for(size_t ich=0; ich<nch; ich++)
    {
      TCanvas *c=new TCanvas("c"+ch[ich]+dist,"c"+ch[ich]+dist,800,800);
      c->Divide(1,nprofs);

      TCanvas *cratios=new TCanvas("cratios"+ch[ich]+dist,"cratios"+ch[ich]+dist,800,800);
      cratios->Divide(1,nprofs);

      TCanvas *cproj=new TCanvas("cproj"+ch[ich]+dist,"cproj"+ch[ich]+dist,600,600);
      TLegend *dataprojLeg=new TLegend(0.6,0.9,0.7,0.7,"Data","brNDC");
      ...
      for(size_t i=0; i<nprofs; i++)
	{
	  ...
	  TH2 *MC = (TH2 *) inF->Get("t#bar{t}/"+ch[ich]+"_"+profs[i]) ; 
	  ...
	  /*
	  TH2 *MC = (TH2 *) inSystF->Get(systList[1]+"/"+ch[ich]+"_"+profs[i]) ; 
	  Double_t totalTTbar(MC->Integral());
	  MC->Add(bckgMC);
	  MC->SetDirectory(0);
	  MC->Sumw2();	  
	  */
	  ...
	  for(size_t isyst=0; isyst<=systMC.size(); isyst++)
	      ...
	      for(int ip=0; ip<DataProf->GetN(); ip++)
		{
	      ...
	      prof->SetTitle( isyst==0 ? "MG+PY" : systLabels[isyst-1] );
	  ...
	  if(i==0)
	    {
	      drawCMSHeader(ch[ich]);

	      TPaveText *pt=new TPaveText(0.9,0.7,1.0,0.9,"brNDC");
	      pt->SetBorderSize(0);
	      pt->SetFillStyle(0);
	      pt->SetTextFont(42);
	      pt->SetTextAlign(12);
	      ...
	      TLegend *leg=new TLegend(0.9,0.5,1.0,0.7);
	      leg->SetBorderSize(0);
	      leg->SetFillStyle(0);
	      leg->SetTextFont(42);
	  ...
	  if(i==0)
	    {
	      ...
	      pt=new TPaveText(0.9,0.7,1.0,0.9,"brNDC");
	      pt->SetBorderSize(0);
	      pt->SetFillStyle(0);
	      pt->SetTextFont(42);
	      pt->SetTextAlign(12);
	      ...
	      TLegend *leg=new TLegend(0.9,0.35,1.0,0.7);
	      leg->SetBorderSize(0);
	      leg->SetFillStyle(0);
	      leg->SetTextFont(42);
	      leg->SetTextSize(0.09);
