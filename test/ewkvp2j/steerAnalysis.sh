...
if [ "$step" == "3" ]; then
    ...
    runLocalAnalysisOverSamples.py -e runVBFZAnalysis -j data/vbfz_photon_samples.json -d ${indir} -o ${outdir}/g/data/qt_tight -c ${cfg} -p "@runSystematics=False @useMVA=True @weightsFile='${outdir}/tight_gamma_weights.root'" -s 8nh -t Data
    runLocalAnalysisOverSamples.py -e runVBFZAnalysis -j data/vbfz_photon_samples.json -d ${indir} -o ${outdir}/g/data/qt_loose -c ${cfg} -p "@runSystematics=True @useMVA=True @weightsFile='${outdir}/loose_gamma_weights.root'"  -s 8nh -t Data
fi

if [ "$step" == "4" ]; then
    echo "Running final plotter"
    runPlotter --iLumi 19736 --inDir ${outdir}/g/data/qt_tight/ --json data/vbfz_photon_samples.json --outFile ${outdir}/plotter_g_tight_qt.root
    runPlotter --iLumi 19736 --inDir ${outdir}/g/data/qt_loose/ --json data/vbfz_photon_samples.json --outFile ${outdir}/plotter_g_loose_qt.root
fi
