...
void regularizeTail(TH1 *h)
{
  TString name(h->GetName());
  if(!name.Contains("met_met") && !name.Contains("_mt")) return;
  if(name.Contains("blind")) return;

  Double_t preint=h->Integral();
  if(preint<=0) return;
  Int_t maxBin=h->GetMaximumBin();
  Float_t xForMax=h->GetBinCenter(maxBin);
  
  //   Float_t xmin=h->GetXaxis()->GetXmin();

  Float_t xmin(xForMax);
  Float_t xmax=h->GetXaxis()->GetXmax(); 
  
  //   TF1 *ffunc=new TF1("ffunc","[0]*x*pow(1.0+[1]*(x-[3])/[2],[2])",0,1e3);
  //   ffunc->SetParLimits(0,0,h->GetMaximum());
  //   ffunc->SetParLimits(1,-1,0);
  //   ffunc->SetParLimits(2,-15,0);
  // ffunc->SetParLimits(3,xForMax*0.75,xForMax*1.25);
    TF1 *ffunc=new TF1("ffunc","[0]*pow(x,[1])",xmin,xmax);
   h->Fit(ffunc,"QEMR0","",xmin,xmax);
 ffunc->SetParLimits(1,-100,0);

   //  TF1 *ffunc=new TF1("ffunc","exp(-[0]*x)*[1]",xmin,xmax);
   //  ffunc->SetParLimits(0,0,1000);
   // ffunc->SetParLimits(1,0,1e9);
   //  h->Fit(ffunc,"QEMR0","",xmin,xmax);

  for(Int_t ibin=maxBin; ibin<=h->GetXaxis()->GetNbins(); ibin++)
    {
      Double_t yval=h->GetBinContent(ibin);
      if(yval>0) continue;
      cout << "\t" << yval << " ";
      Double_t xval=h->GetBinCenter(ibin);
      yval=ffunc->Eval(xval);
      if(yval<0) continue;
      cout << yval << endl;
      h->SetBinContent(ibin, yval );
      h->SetBinError(ibin,sqrt(yval));
    }
  Double_t postint=h->Integral();
  cout << name << " " << preint << " ---> " << postint << " " << ffunc->GetParameter(0) << " " << ffunc->GetParameter(1) << endl;
  //  h->Scale(preint/postint);
  delete ffunc;
}
  ...
  if(model==VBFZ) 
    {
      ...
      gammaFile="/afs/cern.ch/user/p/psilva/work/vbfz/plotter_vbfz_gamma_qt_2012.root";
      llFile="/afs/cern.ch/user/p/psilva/work/vbfz/plotter_vbfz_2012.root";
  ...
  for(std::map<string,Shape_t>::iterator it=shapesMap.begin(); it!=shapesMap.end(); it++) 
   {
     ...
     if(it->first.find("mt_shapes")!= string::npos  
	|| it->first.find("mt_redMet_shapes")!= string::npos 
	/*|| it->first.find("_mt")!= string::npos*/
	|| it->first.find("met_") != string::npos 
	|| it->first.find("dijet_mass_shapes")!=string::npos 
	|| it->first.find("dijet_deta_shapes")!=string::npos
	|| it->first.find("Fisher_shapes")!=string::npos
	|| it->first.find("LikelihoodD_shapes")!=string::npos
	|| it->first.find("BDTD_shapes")!=string::npos
	)
       {
	 ...
	 if(closeF && 
	     ...
	     ))
	   {
	     ...
	     if(it->first.find("Fisher_shapes")!=string::npos) key="Fisher_func";
	     if(it->first.find("LikelihoodD_shapes")!=string::npos) key="LikelihoodD_func";
	     if(it->first.find("BDTD_shapes")!=string::npos) key="BDTD_func";
		 ...
		 for(int jbin=1; jbin<=corrGammaH->GetYaxis()->GetNbins(); jbin++)
		   {
		     ...
		     //set small number
		     if(val<=0){
		       corrGammaH->SetBinContent(ibin,jbin,1e-5);
		       corrGammaH->SetBinError(ibin,jbin,1e-5);
		     }
		   }
	       }
	   }
...
void showShape(const Shape_t &shape,TString SaveName,bool is2011,int model, TFile *closeF)
{
  ...
  if(model!=VBFZ || SaveName.Contains("Likelihood") || SaveName.Contains("Fisher") || SaveName.Contains("BDTD") ) t1->SetLogy(true);
  ...
  if(closeF)
    {
      ...
      closureUnc = (TGraph *)closeF->Get("mumu"+key+"_totalunc");
      ...
      if(model==VBFZ && !SaveName.EndsWith("50"))
	{
	  ...
	  for(int ibin=1; ibin<=instrBckg->GetXaxis()->GetNbins(); ibin++)
	      ...
	      if(closureUncGr)
		{
		  ...
		  //get the uncertainty from the closure tests
		  ...
		  /*
		    ...
		    relUnc = yclose >0 ? closureUnc->GetErrorY(iclose)/yclose : 0;
		    relUnc = sqrt( relUnc*relUnc + pdfRelUnc*pdfRelUnc);
		  */

		  //update uncertainty
		  Double_t relUnc(pow(1-pVal,2));
		  if(y>0) relUnc += pow(yerr/y,2);
		  relUnc=sqrt(relUnc);
		  yerr= y*relUnc;
  		}
  ...
  if(shape.data && mc)
      ...
      for(int ip=0; ip<mcgr->GetN(); ip++)
	{
      ...
      denRelUnc->Draw("3");
      denRelUnc->GetYaxis()->SetRangeUser(0.5,1.5);
