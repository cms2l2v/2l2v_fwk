...
//         char limitlegend[1024]; sprintf(limitlegend, "C'=%3.1f BRnew=%3.1f", CP, BR);
         char limitlegend[1024]; sprintf(limitlegend, "#Gamma = %3.1f GeV", CP);
         ...
         double XSecScaleFactor = 1; //pow(CP,2) * (1-BR);
...
//         g2dMassVsCp   [i]->SetHistogram(histGrid);
	 ...
	 char filePath[1024]; sprintf(filePath, "%s_cp%4.2f_brn%4.2f/Stength_LimitSummary", Dir.c_str(), 100.0, 0.0);
