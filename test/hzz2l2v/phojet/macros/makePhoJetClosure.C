...
void closureTest(TFile *llF,TFile *gF,TString distr,TString ch, TString cat, bool purePhoton)
{
  ...
  if(ch=="ll")
    {
      hdy=(TH1 *) llF->Get(mcdy+"/ee"+cat+"_"+distr);
      hdy2=(TH1 *) llF->Get(mcdy2+"/ee"+cat+"_"+distr);
      hdy->Add(hdy2);
  ...
  std::vector<TString> mcg;
  mcg.push_back("GJets_HT-40to100");
  mcg.push_back("GJets_HT-100to200");
  mcg.push_back("GJets_HT-200to400");
  mcg.push_back("GJets_HT-400to600");
  mcg.push_back("GJets_HT-600toInf");
  
  // mcg.push_back("gammajet");
  if(!purePhoton) mcg.push_back("Multijets");
  TH1 *hg=0, *hpureg=0;
  for(size_t ig=0; ig<mcg.size(); ig++)
    {
	...
	{
	  if(hg) {
	    std::cout << "Looking at histo1 : " << mcg[ig]+"/mumu"+cat+"_"+distr << std::endl;
	    hg->Add( (TH1 *)gF->Get(mcg[ig]+"/mumu"+cat+"_"+distr) );
	  }
	  else{
	    std::cout << "Looking at histo2 : " << mcg[ig]+"/mumu"+cat+"_"+distr << std::endl;
	    hg=(TH1 *)gF->Get(mcg[ig]+"/mumu"+cat+"_"+distr);
	    hg=(TH1 *)hg->Clone("mcg_"+cat+"_"+distr);
	  }
	  hg->Add( (TH1 *)gF->Get(mcg[ig]+"/ee"+cat+"_"+distr) );
