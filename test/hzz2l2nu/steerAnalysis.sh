...
if [ "$step" == "1" ]; then
    ...
    runLocalAnalysisOverSamples.py -e runHZZ2l2nuAnalysis -j data/htozz_samples.json        -d ${indir} -o ${outdir}       -c ${cfg} -p "@runSystematics=False @useMVA=False @weightsFile='${CMSSW_BASE}/src/UserCode/llvv_fwk/data/weights/'" -s 8nh 
...
if [ "$step" == "2" ]; then
    ...
    runPlotter --iLumi 19736 --inDir ${outdir}        --json data/htozz_samples.json        --outFile ${outdir}/plotter.root       --forceMerged #--noPlot
    runPlotter --iLumi 19736 --inDir ${outdir}/g/raw/ --json data/htozz_photon_samples.json --outFile ${outdir}/plotter_g_raw.root --forceMerged #--noPlot
