...
{   
  ...
  TChain *signal     = new TChain("chHiggs");
  TChain *background = new TChain("chHiggs");
  ...
  if (files) {
    ...
    while ((file=(TSystemFile*)next())) {
      fname = file->GetName();
      if(!fname.EndsWith("_mvaTuple.root")) continue;
      //if(fname.Contains("Data")) continue;
      //if(!fname.Contains("DY")) continue;
      if(!fname.Contains("TTJets") && !fname.Contains("TBH_M-300") && !fname.Contains("HTB_M-300")) continue;
      bool isSignal(false);
      if(fname.Contains("HTB") || fname.Contains("TBH")) { signal->Add(fname); isSignal=true; }
      //else if(fname.Contains("50toInf") && fname.Contains("DY")) background->Add(fname);
      else background->Add(fname);
  ...
  if(minimalTrain)
    {
      factory->AddVariable( "nbjets",     "b-jet multiplicity"        "",    'I' );
      factory->AddVariable( "leadbjetpt", "p_{T}^{leading b-jet}",    "GeV", 'F' );
      factory->AddVariable( "njets",      "jet multiplicity"          "",    'I' );
      factory->AddVariable( "globalmt",   "M_{T}(ll'bbE_{T}^{miss})", "GeV", 'F' );
      factory->AddVariable( "met",        "E_{T}^{miss}",             "GeV", 'F' );
    }
  else
    {
      factory->AddVariable( "nbjets",     "b-jet multiplicity"        "",    'I' );
      ...
      factory->AddVariable( "detall",     "#Delta#eta_{ll}",          "",    'F' );
      factory->AddVariable( "dphill",     "#Delta#phi(ll)",           "",    'F' );
